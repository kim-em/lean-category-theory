...
@[unfoldable] definition {u v} Surjective { α : Type u } { β : Type v } ( f : α → β ) : Prop := nonempty { g : β → α // g ∘ f = id }
@[unfoldable] definition {u v} Injective  { α : Type u } { β : Type v } ( f : α → β ) : Prop := Π { X Y : α }, (f X = f Y) → (X = Y)
...
@[unfoldable] definition {u1 v1 u2 v2} Embedding { C : Category.{u1 v1} } { D : Category.{u2 v2} } ( F : Functor C D ) := (Full F) ∧ (Faithful F)
