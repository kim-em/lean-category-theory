...
import category_theory.universal.uniqueness
...
structure compatible_sections (c : cover X) (F : (open_set X)ᵒᵖ ⥤ (Type u)) :=
  (sections      : Π i : c.I, F.obj (c.U i))
  (compatibility : Π i j : c.I, res_left i j F (sections i) = res_right i j F (sections j))

structure gluing {c : cover X} {F : (open_set X)ᵒᵖ ⥤ (Type u)} (s : compatible_sections c F) :=
