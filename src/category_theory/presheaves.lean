...
begin
  ...
  dsimp at h',
  dsimp [open_set.map_iso, whisker_on_right, whiskering_on_right, nat_iso.of_components, nat_trans.hcomp] at h',
  simp at h',
  dsimp at h',
...
{ f := 𝟙 F.X,
  c := ((functor.id_comp _).inv) ⊟ (whisker_on_right (open_set.map_id _).inv _) }

def comp {F G H : Presheaf.{u v} C} (α : Presheaf_hom F G) (β : Presheaf_hom G H) : Presheaf_hom F H :=
{ f := α.f ≫ β.f,
  c := β.c ⊟ (whisker_on_left (open_set.map β.f) α.c) }
  ...
  begin
    ...
    { ext1,
      dsimp only [Presheaf_hom.comp,
             whisker_on_right, whiskering_on_right, whisker_on_left, whiskering_on_left,
