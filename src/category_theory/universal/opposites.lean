...
-- def initial_object_in_opposite (i : initial_object.{u₁+1 u₁} (Cᵒᵖ)) : terminal_object.{u₁+1 u₁} C := 
-- { obj := i.obj,
--   «from» := i.to,
--   uniqueness := i.uniqueness }

-- def terminal_object_in_opposite (t : terminal_object.{u₁+1 u₁} (Cᵒᵖ)) : initial_object.{u₁+1 u₁} C := 
-- { obj := t.obj,
--   to := t.«from»,
