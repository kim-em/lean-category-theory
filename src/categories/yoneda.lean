...
section
variables (C : Type u₁) [uv_category.{u₁ v₁} C]

-- FIXME why isn't this already available?
instance : uv_category ((Cᵒᵖ) ↝ Type v₁ × (Cᵒᵖ)) := products.ProductCategory.{(max u₁ (v₁+1)) (max u₁ v₁) u₁ v₁} (Cᵒᵖ ↝ Type v₁) (Cᵒᵖ)
...
{ onObjects := λ X, 
    { onObjects     := λ Y, @uv_category.Hom C _ Y X,
...
begin
  letI : uv_category (Cᵒᵖ × (Cᵒᵖ ↝ Type v₁)) := by apply_instance,
  exact (F ⋙ G ⋙ H)      
end

definition CoYoneda (C : Type u₁) [uv_category.{u₁ v₁} C] : (Cᵒᵖ) ↝ (C ↝ (Type v₁)) := 
{ onObjects := λ X, 
   { onObjects     := λ Y, @uv_category.Hom C _ X Y,
...
@[simp] private lemma YonedaLemma_aux_1 {C : Type u₁} [uv_category.{u₁ v₁} C] {X Y : C} (f : X ⟶ Y) {F G : (Cᵒᵖ) ↝ (Type v₁)} (τ : F ⟹ G) (Z : F +> Y) :
...
theorem YonedaLemma (C : Type u₁) [uv_category.{u₁ v₁} C] : (YonedaPairing C) ⇔ (YonedaEvaluation C) := 
{ morphism := { components := λ F x, ulift.up ((x.components F.2) (𝟙 F.2)) },
  inverse  := { components := λ F x, { components := λ X a, (F.1 &> a) x.down } } }.

theorem YonedaFull (C : Type u₁) [uv_category.{u₁ v₁} C] : Full (Yoneda C) := 
...
theorem YonedaFaithful (C : Type u₁) [uv_category.{u₁ v₁} C]  : Faithful (Yoneda C) := {
