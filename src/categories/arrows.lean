...
@[applicable] lemma arrow_hom_equal {C : Type (u₁+1)} [category C] (f g : arrow C) (α β : arrow_hom f g) (w : α.morphism = β.morphism) : α = β :=
begin
...
@[reducible] instance Arrows (C : Type (u₁+1)) [category C] : category (arrow C):=
{ Hom := arrow_hom,
  identity       := by tidy,
