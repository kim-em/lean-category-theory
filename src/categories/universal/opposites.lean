...
def Coequalizer_from_Equalizer_in_Opposite         (e : @Equalizer (Cᵒᵖ) Y X f g)   : Coequalizer f g := sorry
def Equalizer_from_Coequalizer_in_Opposite         (e : @Coequalizer (Cᵒᵖ) Y X f g) : Equalizer f g := sorry
def Coproduct_from_Product_in_Opposite             {I: Type u₃} {F : I → C}   (p : @Product (Cᵒᵖ) _ F)         : Coproduct F := sorry
def Product_from_Coproduct_in_Opposite             {I: Type u₃} {F : I → C}   (p : @Coproduct (Cᵒᵖ) _ F)       : Product F := sorry
def BinaryCoproduct_from_BinaryProduct_in_Opposite (p : @BinaryProduct (Cᵒᵖ) X Y)   : BinaryCoproduct X Y := sorry
def BinaryProduct_from_BinaryCoproduct_in_Opposite (p : @BinaryCoproduct (Cᵒᵖ) X Y) : BinaryProduct X Y := sorry

def Cones_in_Opposite   (F : Functor J C) : Equivalence (Cones (OppositeFunctor F)) (Cocones F) := sorry
def Cocones_in_Opposite (F : Functor J C) : Equivalence (Cocones (OppositeFunctor F)) (Cones F) := sorry
...
def Cocomplete_of_Opposite_Complete (C : Type u₁) [category C] [Complete (Cᵒᵖ)]   : Cocomplete C := sorry
def Complete_of_Opposite_Cocomplete (C : Type u₁) [category C] [Cocomplete (Cᵒᵖ)] : Complete C := sorry
